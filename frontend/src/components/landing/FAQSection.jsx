    ...
    <section id="faq" className="bg-[#020617] py-24 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col gap-3">
          {faqs.map((faq, index) => (
            ...
            >
              ...
              >
                <span className={`text-sm sm:text-base font-semibold transition-colors duration-200 ${openIndex === index ? 'text-cyan-400' : 'text-white group-hover:text-cyan-400'}`}>
                  {faq.q}
                </span>
                <ChevronDown
                  size={18}
                  className={`text-slate-400 flex-shrink-0 ml-4 transition-transform duration-300 ${openIndex === index ? 'rotate-180 text-cyan-400' : ''}`}
                />
              </button>
        ...
        <div className="text-center mt-10 flex flex-col items-center gap-4">
          ...
          <a href="#pricing" className="px-6 py-2.5 rounded-xl border border-white/15 text-slate-300 text-sm font-semibold hover:border-cyan-500/30 hover:text-cyan-400 transition-all duration-200">
            Start Free
          </a>
        </div>
