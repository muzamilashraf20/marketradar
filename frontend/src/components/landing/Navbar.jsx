    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 px-6 h-16 flex items-center justify-between transition-all duration-300 ${scrolled ? "bg-[#030712]/95 backdrop-blur-lg border-b border-cyan-500/10" : "bg-transparent"}`}>
        ...
        <div className="hidden md:flex items-center gap-10">
          {links.map((link) => (
            <button key={link.id} onClick={() => scrollToSection(link.id)} className="text-slate-400 hover:text-white transition-colors">
              {link.name}
            </button>
          ))}
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-[#030712] flex flex-col items-center justify-center md:hidden">
          <button onClick={() => setMenuOpen(false)} className="absolute top-5 right-6 text-white">
            <X size={28} />
          </button>
          <a href="/landing" onClick={() => setMenuOpen(false)} className="absolute top-4 left-6 flex items-center gap-2">
          ...
          <div className="flex flex-col items-center gap-8">
            {links.map((link) => (
              <button key={link.id} onClick={() => scrollToSection(link.id)} className="text-3xl font-bold text-slate-300 hover:text-cyan-400 transition-colors">
                {link.name}
              </button>
            ))}
