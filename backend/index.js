...
app.post('/api/checkout', async (req, res) => {
  ...
  try {
    const response = await axios.post('https://api.lemonsqueezy.com/v1/checkouts', { data: { type: 'checkouts', attributes: { checkout_data: {}, product_options: { redirect_url: 'https://marketradar-taupe.vercel.app/?payment=success' } }, relationships: { store: { data: { type: 'stores', id: String(LS_STORE_ID) } }, variant: { data: { type: 'variants', id: String(plan.variantId) } } } } }, { headers: { 'Authorization': `Bearer ${LS_API_KEY}`, 'Content-Type': 'application/vnd.api+json', 'Accept': 'application/vnd.api+json' } })
...
app.post('/api/ai', async (req, res) => {
  ...
  try { const m = await anthropic.messages.create({ model: 'claude-sonnet-4-5-20250514', max_tokens: 1024, system: system || 'You are a financial markets analyst for BiasForge.ai.', messages: [{ role: 'user', content: prompt }] }); res.json({ success: true, response: m.content[0].text }) } catch (e) { res.status(500).json({ error: 'AI failed' }) }
})
app.post('/api/bias', async (req, res) => {
  ...
  try { const m = await anthropic.messages.create({ model: 'claude-sonnet-4-5-20250514', max_tokens: 2048, system: 'You are a senior macro trader for BiasForge.ai. Return ONLY valid JSON.', messages: [{ role: 'user', content: `Analyze ${symbol} for ${timeframe || 'intraday'} bias as of ${new Date().toISOString()}.` }] }); res.json({ success: true, bias: JSON.parse(m.content[0].text.trim().replace(/```json|```/g, '').trim()) }) } catch (e) { res.status(500).json({ success: false, error: 'AI analysis failed.' }) }
})
