...
// ============================================
// 🧪 TEMPORARY — web search feasibility probe. DELETE AFTER TESTING.
// Fixed queries only (no caller-supplied prompt) so this is not an arbitrary-prompt hole
// like /api/ai. Exists solely to answer: can the model fetch a release actual from the
// primary source, does it cite it, does the "not available" guard hold, what does it cost.
// ============================================
const PRIMARY_SOURCES = ['bls.gov', 'dol.gov', 'census.gov', 'bea.gov', 'federalreserve.gov', 'adp.com', 'ismworld.org']

const WS_GUARD = `You report economic data releases. Rules you must not break:
...
2. If you cannot find the figure, or you are not certain the number you found is the one asked for, answer exactly: {"status":"not available","reason":"<why>"} — nothing else.
...
4. The release date and the reference period are different things. Report which is which, and do not confuse a revised figure for the initial print.

Return ONLY raw JSON, no markdown:
{"status":"found"|"not available","value":"<figure exactly as published>","reference_period":"<what period it covers>","release_date":"<when published>","source_url":"<exact URL>","source_name":"<publisher>","reason":"<only when not available>"}`

const WS_CASES = {
  // Can it get a real release actual from the primary source?
  claims: {
    label: 'US initial jobless claims (DOL primary source)',
    tools: [{ type: 'web_search_20260209', name: 'web_search', max_uses: 6, allowed_domains: PRIMARY_SOURCES }],
    prompt: 'What is the most recently published US initial jobless claims (seasonally adjusted) figure, and for which week ending? Use the Department of Labor release.',
  },
  // The two the FRED path structurally cannot supply — proprietary, not on any free API.
  ism: {
    label: 'ISM Manufacturing PMI (proprietary — unavailable on FRED)',
    tools: [{ type: 'web_search_20260209', name: 'web_search', max_uses: 6, allowed_domains: PRIMARY_SOURCES }],
    prompt: 'What was the most recent ISM Manufacturing PMI headline index value, and for which month?',
  },
  adp: {
    label: 'ADP National Employment Report (proprietary — unavailable on FRED)',
    tools: [{ type: 'web_search_20260209', name: 'web_search', max_uses: 6, allowed_domains: PRIMARY_SOURCES }],
    prompt: 'What was the most recent ADP National Employment Report private payrolls change, and for which month?',
  },
  // Guard test: a figure that does not exist. Must return "not available", not a fabrication.
  guard: {
    label: 'GUARD TEST — figure that does not exist',
    tools: [{ type: 'web_search_20260209', name: 'web_search', max_uses: 4, allowed_domains: PRIMARY_SOURCES }],
    prompt: 'What was the US Bureau of Labor Statistics "National Barista Sentiment Index" reading for last month?',
  },
  // Does Cloudflare block the fetcher on ForexFactory?
  ff: {
    label: 'ForexFactory reachability (Cloudflare?)',
    tools: [{ type: 'web_fetch_20260209', name: 'web_fetch', max_uses: 3, allowed_domains: ['forexfactory.com', 'www.forexfactory.com'] }],
    prompt: 'Fetch https://www.forexfactory.com/calendar and report whether the page loaded and whether it contains an economic calendar with actual values. If the fetch was blocked or returned an error, say exactly what the error was.',
  },
}

app.get('/api/_websearch-test', aiRateLimiter, async (req, res) => {
  const key = String(req.query.case || '')
  const c = WS_CASES[key]
  if (!c) return res.status(400).json({ error: `case must be one of: ${Object.keys(WS_CASES).join(', ')}` })
  const t0 = Date.now()
  try {
    let messages = [{ role: 'user', content: c.prompt }]
    let m, pauses = 0
    // Server-tool loop can stop at pause_turn (10-iteration server cap) — resume by
    // re-sending with the assistant turn appended. No extra user message.
    for (;;) {
      m = await anthropic.messages.create({
        model: 'claude-sonnet-4-6', max_tokens: 4096, system: WS_GUARD, tools: c.tools, messages,
      })
      if (m.stop_reason !== 'pause_turn' || pauses >= 3) break
      pauses++
      messages = [...messages, { role: 'assistant', content: m.content }]
    }
    trackAI('websearch-test', 'claude-sonnet-4-6', m.usage)

    // Inspect what the server tools actually returned. On success a web_search result's
    // `content` is a LIST of results; on failure it is an OBJECT carrying error_code —
    // branch on that before indexing, or an error looks like an empty result set.
    const searches = [], fetches = [], toolErrors = [], text = []
    for (const b of m.content) {
      if (b.type === 'text') text.push(b.text)
      else if (b.type === 'web_search_tool_result') {
        if (Array.isArray(b.content)) searches.push(...b.content.map(r => ({ title: r.title, url: r.url })))
        else toolErrors.push({ tool: 'web_search', error_code: b.content?.error_code || 'unknown' })
      } else if (b.type === 'web_fetch_tool_result') {
        if (b.content?.error_code) toolErrors.push({ tool: 'web_fetch', error_code: b.content.error_code })
        else fetches.push({ url: b.content?.url || null, retrieved: !!b.content?.content })
      }
    }
    const joined = text.join('\n').trim().replace(/```json|```/g, '').trim()
    let parsed = null
    try { parsed = JSON.parse(joined) } catch (e) {
      const s = joined.indexOf('{'), t = joined.lastIndexOf('}')
      if (s >= 0 && t > s) { try { parsed = JSON.parse(joined.slice(s, t + 1)) } catch (e2) {} }
    }
    const u = m.usage || {}
    const searchCount = u.server_tool_use?.web_search_requests || 0
    const fetchCount = u.server_tool_use?.web_fetch_requests || 0
    // Sonnet 4.6: $3/$15 per MTok. Web search bills separately at $10 per 1,000 searches.
    const tokenUSD = ((u.input_tokens || 0) * 3 + (u.output_tokens || 0) * 15) / 1e6
    const searchUSD = searchCount * 0.01
    res.json({
      case: key, label: c.label,
      elapsedMs: Date.now() - t0,
      stop_reason: m.stop_reason, pauses,
      answer: parsed || { raw: joined.slice(0, 800) },
      sourcesReturned: searches, fetched: fetches, toolErrors,
      usage: { ...u, searchCount, fetchCount },
      costUSD: { tokens: +tokenUSD.toFixed(4), search: +searchUSD.toFixed(4), total: +(tokenUSD + searchUSD).toFixed(4) },
    })
  } catch (e) {
    console.error(`❌ [websearch-test ${key}] ${e?.message || e}`)
    res.status(502).json({ case: key, error: String(e?.message || e).slice(0, 400), elapsedMs: Date.now() - t0 })
  }
})
